      ...
      case "banlist":
        ...
        await ctx.editMessageText(
          banData
            .map(
              (item) =>
                `${item.userId} در ${new Date(item.bannedAt).toLocaleString()} توسط ${item.BannedBy}\n`,
            )
            .join(""),
        );
        break;
      case "silentlist":
        ...
        await ctx.editMessageText(
          silentData
            .map(
              (item) =>
                `${item.userId} در ${new Date(item.silentedAt).toLocaleString()} توسط ${item.silentBy}\n`,
            )
            .join(""),
        );
        break;
      case "adminlist":
        ...
        await ctx.editMessageText(
          adminData
            .map(
              (item) =>
                `${item.userId} در ${new Date(item.promotedAt).toLocaleString()} توسط ${item.PromotedtBy}\n`,
            )
            .join(""),
        );
        break;
